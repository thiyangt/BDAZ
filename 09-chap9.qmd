...
```{r, echo=TRUE,  comment=NA, message=FALSE, warning=FALSE, echo=FALSE}
library(tidyverse)
```
...
```{r, echo=TRUE, comment=NA, fig.width=2, fig.height=3, , fig.cap="Distribution of HDL level", fig.pos="h"}
library(ggplot2)
...
```{r, echo=TRUE, fig.width=3, fig.cap="Distribution of pH values", fig.pos="h", fig.height=4}
...
```{r, echo=TRUE,  comment=NA, fig.cap="Normal probability plot of pH values", fig.height=3, fig.width=3}
ggplot(ph.df, 
       aes(sample=pH))+
...
```{r, echo=TRUE, comment=NA, message=FALSE, warning=FALSE, fig.cap="Distribution of cholesterol levels after and before the special diet"}
